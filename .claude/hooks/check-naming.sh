...
# Номер ISO-недели (`date +%G-W%V`) — часть формата даты, а не произвольный регистр.
# Строчная `w` хук бы прошла, но orz-check.sh сравнивает недели строкой, и
# `2026-w38` больше любого `2026-W..`: незакрытая неделя перестала бы флагаться молча.
case "$base" in
  weekplan-[0-9][0-9][0-9][0-9]-W[0-9][0-9].md) exit 0 ;;
esac
