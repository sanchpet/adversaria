...
> **Номер общий с развёрнутым экземпляром, и пропуск в нумерации значим.** Решение, принятое в экземпляре и не касающееся шаблона, номер занимает, а файла здесь не оставляет. Пропуск не заполняется другим решением: `ADR-NNNN` в скиллах и документах должен означать одно и то же в обоих контурах.
