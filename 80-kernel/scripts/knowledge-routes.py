...
def main():
    ...
    for note in sorted(knowledge.glob("*.md")):
        try:
            ...
            continue
        # fnmatch: '*' пересекает разделитель каталогов, поэтому `.repos*/<group>/<repo>/*`
        # достаёт и рабочее дерево, и любую глубину внутри чарта
        patterns = declared_routes(text)
        # Каталог тоже есть контакт с классом: `git -C .repos/<group>/<repo>` не совпадает
        # с глобом `.repos/<group>/<repo>/*`, но маршрут лежит под ним.
        hits = sorted(
            rel
            for rel in rels
            for p in patterns
